...
\section{Conditions}
\mytable{gX}{
	\specialcell{\code{if \textit{expression}; then} \\ \code{  \textit{do something}} \\ \code{else} \\ \code{  \textit{do something else}}\\ \code{fi}}	&	typical if statement. Expressions can be commands and functions (return 0 $\rightarrow$ true) or builtin conditionals, see below	\\
